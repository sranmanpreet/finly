...
const ChartMonthlyCategory: React.FC<ChartMonthlyCategoryProps> = ({ monthlyCategory }) => (
  ...
  <h2 className="text-base font-semibold mb-1 text-gray-800 dark:text-gray-100">Monthly Category Breakdown</h2>
    <ResponsiveContainer width="100%" height={220}>
      <BarChart data={monthlyCategory}>
        ...
        {Object.keys(monthlyCategory[0] || {})
          .filter(k => k !== "month")
          .map((cat, idx) => (
            <Bar key={cat} dataKey={cat} stackId="a" fill={COLORS[idx % COLORS.length]} barSize={18} name={`₹ ${cat}`} />
        ))}
      </BarChart>
    </ResponsiveContainer>
  </div>
